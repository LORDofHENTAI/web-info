...
export class ProductPriceService {
  ...
  private urlGetShopSections = environment.apiUrl + "printlist/getShopSections/";
  private urlFilterMile = environment.apiUrl + 'printlist/filterMile'
  private urlImportFromExcel = environment.apiUrl + 'printlist/importFromExcel'
  private urlCancelExcel = environment.apiUrl + 'printlist/cancelExcel'
  ...
  getShopSections(): Observable<ShopSection[]> {
    return this.http.get<ShopSection[]>(`${this.urlGetShopSections}`);
  }
  ...
  FilterMile(fileToUpload: PrintUpload): Observable<GetFiltredPrintListModel> {
    let input = new FormData();
    input.append("token", fileToUpload.token);
    input.append("file", fileToUpload.file);
    input.append("actionDate", fileToUpload.actionDate)
    ...
    input.append("shop", fileToUpload.shop);
    input.append("priceType", fileToUpload.priceType);
    return this.http.post<GetFiltredPrintListModel>(this.urlFilterMile, input)
  }
  ImportFromExcel(data: GetFiltredPrintListModel): Observable<Status> {
    return this.http.post<Status>(this.urlImportFromExcel, data)
  }
  CancelExcel(data: GetFiltredPrintListModel): Observable<Status> {
    return this.http.post<Status>(this.urlCancelExcel, data)
  }
}